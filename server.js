...
io.on("connection", function (socket) {
  ...
  socket.on("change_password", function (data) {
    io.sockets.emit("change_password", {
      user: data.user,
    });
  ...
  for (const elem of market_types) {
    ...
    let runners = await query(
      "SELECT * FROM `market_book_odds_runner` WHERE `market_id` = '" +
        ...
        "' AND `event_id` = '" +
        event_id +
        "' and market_book_odds_runner.updated_at >=DATE_SUB(NOW(),INTERVAL 1 HOUR) ORDER BY `id` ASC"
    );
...
getListEvents = () => {
  return new Promise((resolve, rejector) => {
    query(
      "SELECT le.competition_id, le.event_type, le.event_id, le.event_name, le.open_date from `list_events` as `le` where  le.updated_at >=DATE_SUB(NOW(),INTERVAL 1 HOUR) "
    )
      .then((player) => {
        resolve(player);
      })
  ...
  for (const elem of list_event) {
    ...
    let market_book_odds_fancy = await query(
      "SELECT id,match_id,selection_id,runner_name,lay_price1,lay_size1,back_price1,back_size1,game_status,mark_status,is_active,cron_disable from market_book_odds_fancy where match_id='" +
        event_id +
        "' and is_active = 'Yes' and  runner_name NOT LIKE  '%BALL RUN%'"
    );

    var get_fancy_block_market = await getFancyBlockMarket(
      market_book_odds_fancy
    );
    elem["fancy_data"] = get_fancy_block_market;
...
app.get("/getMarketBookOddsFancy", async (req, res) => {
  ...
  res.send(
    JSON.stringify({
      status: 200,
      error: null,
      response: market_book_odds_fancy,
    })
  );
});
...
app.post("/casino-score-data", async (req, res) => {
  var data = "";
